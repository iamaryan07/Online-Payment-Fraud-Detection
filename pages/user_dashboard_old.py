...
def process_payment(sender_id, recipient_id, amount, description, device_data, location="US", failed_attempts=0, test_scenario="Normal Transaction"):
    """Enhanced payment processing with AUTOMATIC fraud test scenario selection - BASED ON AMOUNT"""
    try:
        eng = get_engine()
        ...
        # Calculate comprehensive risk score with test scenario
        risk_score, risk_factors = calculate_fraud_risk_score(
            amount, sender_id, recipient_id, location, device_data, failed_attempts, test_scenario
        )
        
        # AUTOMATIC categorization based on risk score
        if risk_score >= block_threshold:
            status = "Pending Approval"  # High risk - block for investigation
            process_now = False
        elif risk_score >= flag_threshold:
            status = "Under Review"  # Medium risk - process but flag
            process_now = True
        else:
            status = "Success"  # Low risk - process immediately
            process_now = True
        
        # Get user balances and recipient info
        with eng.begin() as conn:
            ...
            if not sender or not recipient:
                return None, "Error: User not found", 0.0, [], None
            
            if sender.balance < amount:
                return None, "Insufficient Balance", 0.0, [], None
        ...
        # Create comprehensive transaction record with test scenario details
        now = datetime.now()
        ...
        transaction_details = {
            ...
            "test_scenario": test_scenario,  # Track the test scenario used
            "risk_analysis": {
                "flag_threshold": flag_threshold,
                "block_threshold": block_threshold,
                "automated_decision": status,
                "test_scenario_applied": test_scenario
            },
            "fraud_indicators": {
                "high_amount": amount > sys_cfg.get("tx_limit_amount", 5000.0),
                "unusual_location": location in ["Unknown", "High-Risk-Geo"],
                "suspicious_device": any(word in device_data.get("user_agent", "").lower() 
                                       for word in ["emulator", "bot", "headless"]),
        ...
        with eng.begin() as conn:
            ...
            # Create investigation case for high-risk transactions - AUTOMATIC
            ...
            # Enhanced audit logging with test scenario tracking
            audit_details = f"Payment processing: ${amount:.2f} from user {sender_id} to user {recipient_id} - Status: {status} - Risk: {risk_score:.3f} - Test Scenario: {test_scenario} - Factors: {len(risk_factors)}"
        ...
        return tx_id, status, risk_score, risk_factors, recipient.name
        
    except Exception as e:
        st.error(f"Payment processing error: {str(e)}")
        return None, f"Error: {str(e)}", 0.0, [], None
